---
name: harness-init
description: One-time, guided setup of the local agent-first coding harness — the polling loop, the deterministic dispatcher, its config, the CLAUDE.md contract, and the worktree provisioning that makes per-feature worktrees runnable. Use when a developer wants to set up, bootstrap, install, or initialize the coding harness for the first time in a project.
---
...
- **Canonical** (drop in verbatim from `assets/`): the dispatcher, the shim
  skill, `REVIEW.md`, the env template, the CLAUDE.md template skeleton, the
  review workflow. Show them, explain them, invite tweaks — but they're the same
  everywhere.
- **Software 3.0** (generated by reading the repo): `CLAUDE.md`'s filled-in
...
**bootstrap hook** (the one deviation from the canonical doc dispatcher; explain
why it's there). Offer the common tweaks (PRD-runner stuck cap, feedback round
...
### Step 4 — `CLAUDE.md` (Software 3.0)

Read `references/claude-md-guidance.md` and `references/project-discovery.md`.
Scan the repo, fill `assets/CLAUDE.md.template`'s bracketed parts (project name,
the verification-layer tooling), keep the rest verbatim. Present it as a diff
against the template, citing the scan finding behind each filled-in line. Let the
user edit before committing. Tell them the Expert section is intentional even
...
4. Review and merge the `/expert-update` PR that follows.
...
`/implement-mainspec`, `/fix-local-checks`, `/address-feedback`, and
`/expert-update`. harness-init wires the **outer** harness; it does not author
...
and offer to update. For generated files (`CLAUDE.md`, `local-checks.sh`,
