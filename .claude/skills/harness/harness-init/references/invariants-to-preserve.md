...
| 5 | **Dispatcher discipline** | Zero LLM calls in the dispatcher. One step per branch per tick. `claude -p` invocations are synchronous, never backgrounded with `&`. `flock -n` serializes ticks — and the host-worktree **sync** in `harness-tick.sh` is serialized against an in-flight `/learn` via `.harness/learn-running`, so the per-tick force-sync never resets the worktree out from under the one skill that runs in it. |
